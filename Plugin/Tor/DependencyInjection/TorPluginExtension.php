...
/**
 * Class TorPluginExtension.
 */
class TorPluginExtension extends BaseExtension
{
    /**
    ...
    {
        return new TorPluginConfiguration($this->getAlias());
    }
    ...
    {
        $sources = Env::getArray('TOR_IPS_SOURCES', $config['sources'] ?? [], true);

        return [
            'apisearch_plugin.tor.sources' => $sources,
            'apisearch_plugin.tor.populate_interval' => Env::get('TOR_POPULATE_INTERVAL', $config['populate_interval'] ?? [], false),
        ];
    }
}
